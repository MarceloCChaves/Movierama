...
const ListMovies = () => {
  const favorites = useSelector((state: RootState) => {
    const favoritesReduce = state.favorites.reduce((itens: IMovie[], favoriteItens: IMovie) => {
      const item = state.favorites.find((item) => item.id === favoriteItens.id);
      itens.push({
        ...item,
        id: favoriteItens.id,
  ...
  return (
    <>
      <h1 className={styles.title}>Filmes Favoritos</h1>
